...
prop.table(table(train$gender, train$`Class/ASD`), 1)
prop.table(table(train$jaundice, train$`Class/ASD`), 1)
prop.table(table(train$austim, train$`Class/ASD`), 1)
prop.table(table(train$used_app_before, train$`Class/ASD`), 1)
prop.table(table(train$`Class/ASD`)) * 100

prop.table(table(train$ethnicity, train$`Class/ASD`), 1)
prop.table(table(train$contry_of_res, train$`Class/ASD`), 1)
prop.table(table(train$age_desc, train$`Class/ASD`), 1)
prop.table(table(train$relation, train$`Class/ASD`), 1)
...
train$`Class/ASD` <- factor(train$`Class/ASD`, levels = c(0, 1), labels = c("No ASD", "ASD"))

ggplot(train, aes(x = `Class/ASD`, y = correct_result, fill = `Class/ASD`)) +
  geom_violin(trim = FALSE, alpha = 0.7, color = NA) +
  geom_boxplot(width = 0.1, outlier.shape = NA, alpha = 0.8, color = "black") +
  stat_summary(fun = "median", geom = "point", shape = 21, size = 2, color = "white", fill = "black") +
  scale_fill_manual(values = c("#FF9999", "#66CCFF")) +
  labs(
    title = "Distribution of AQ Score by ASD Diagnosis",
    x = "ASD Class",
    y = "AQ Score (correct_result)"
  ) +
  theme_minimal(base_size = 13) +
  theme(legend.position = "none")


ggplot(train, aes(x = ethnicity, fill = `Class/ASD`)) +
  geom_bar(position = "dodge") +
  coord_flip() +
  labs(
    title = "ASD Diagnosis Count by Ethnicity (Horizontal)",
    x = "Ethnicity",
    y = "Count",
    fill = "ASD Diagnosis"
  ) +
...
df_prop <- train %>%
  group_by(austim, `Class/ASD`) %>%
  summarise(n = n()) %>%
  group_by(austim) %>%
  mutate(prop = n / sum(n))

ggplot(df_prop, aes(x = austim, y = prop, fill = `Class/ASD`)) +
  ...
  labs(
    title = "ASD Diagnosis Proportions by Family Autism History",
    x = "Family Autism History",
    y = "Proportion",
    fill = "ASD Diagnosis"
  ) +
  theme_minimal(base_size = 13)
...
df_prop_1 <- train %>%
  group_by(jaundice, `Class/ASD`) %>%
  summarise(n = n()) %>%
  group_by(jaundice) %>%
  mutate(prop = n / sum(n))

ggplot(df_prop_1, aes(x = jaundice, y = prop, fill = `Class/ASD`)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_text(aes(label = scales::percent(prop, accuracy = 1)),
            position = position_dodge(width = 0.9),
            vjust = -0.3, size = 4) +
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "ASD Diagnosis Proportions by Jaundice History",
    x = "Jaundice History",
    y = "Proportion",
    fill = "ASD Diagnosis"
  ) +
...
model_data_clean$Class <- factor(model_data_clean$Class, levels = c("NoASD", "ASD"))
...
confusionMatrix(svm_model$pred$pred, svm_model$pred$obs, positive = "ASD")
...
print(ann_model)
plot(ann_model)
confusionMatrix(ann_model$pred$pred,
                ann_model$pred$obs,
                positive = "ASD")
ann_model$bestTune
...
print(rf_model)
rf_model$bestTune
confusionMatrix(rf_model$pred$pred, rf_model$pred$obs, positive = "ASD")
plot(rf_model)
...
print(xgb_model)
confusionMatrix(xgb_model$pred$pred, xgb_model$pred$obs, positive = "ASD")
plot(xgb_model)


# train performance

train_pred_prob <- predict(xgb_model, newdata = model_data_clean, type = "prob")[, "ASD"]
train_pred_class <- ifelse(train_pred_prob > 0.5, "ASD", "NoASD")
train_pred_class <- factor(train_pred_class, levels = levels(model_data_clean$Class))

confusionMatrix(train_pred_class, model_data_clean$Class, positive = "ASD")

roc_obj_train <- roc(model_data_clean$Class, train_pred_prob, levels = c("NoASD", "ASD"))
auc(roc_obj_train)


# feature importance

xgb_matrix <- model.matrix(Class ~ . - 1, data = model_data_clean)

model_features <- xgb_model$finalModel$feature_names

xgb_matrix_used <- xgb_matrix[, model_features]

importance_matrix <- xgb.importance(feature_names = model_features,
                                    model = xgb_model$finalModel)

xgb.plot.importance(importance_matrix,
                    top_n = 10,
                    main = "Top 10 Feature Importances - XGBoost")
